...
const DropdownProfile = () => {
  return (
    <div className="absolute -bottom-[100px] md:-bottom-[132px] -right-2 md:-right-5 z-20 w-32 md:w-[156px] rounded-sm py-1 font-lato text-white bg-pageHeaderBackground">
        <div className="flex items-center w-full  hover:bg-extraBackground transition duration-700 gap-1.5 px-3 py-2">
            < IoPerson className="text-sm md:text-lg" />
            <Link to="" className="text-xs md:text-sm">Profil Saya</Link>
        </div>
        <div className="flex items-center w-full  hover:bg-extraBackground transition duration-700 gap-1.5 px-3 py-2">
            < MdOutlineStar className="text-sm md:text-lg" />
            <Link to="" className="text-xs md:text-sm">Ubah Premium</Link>
        </div>
        <div className="flex items-center w-full  hover:bg-extraBackground transition duration-700 gap-1.5 px-3 py-2">
            < RiLogoutBoxRLine className="text-sm md:text-lg" />
            <Link to="/" className="text-xs md:text-sm">Keluar</Link>
        </div>
    </div>
  )
}
